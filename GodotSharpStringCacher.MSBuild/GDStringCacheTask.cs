...
public class GDStringCacheTask : Task
{
    ...
    {
        Config defaultConfig = new(UseShortNamesByDefault);
        ...
        foreach (ITaskItem reference in ReferencePath)
        {
            ...
            if (GetBoolMetadata(reference, "CacheStrings"))
            {
                assemblyTask = reference;
            }
    ...
    static Config ParseConfig(ITaskItem taskWithOptions, Config defaultConfig)
    {
        ...
        return new Config(GetBool("ShortNames", defaultConfig.ShortNames));
    }
