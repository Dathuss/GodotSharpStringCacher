...
```xml
<PackageReference Include="GodotSharpStringCacher.MSBuild" PrivateAssets="all" Version="3.0.0" />
```
...
```xml
<PackageReference Include="GodotSharpStringCacher" Version="3.0.0" />
```
Then:
```csharp
...
ctx.RunAndSave("input2.dll", "output2.dll", out string? outputPdbFile);
```
...
```
GodotSharpStringCacher.Console <in_file> <out_file> [--long-names] [--godotsharp-path=PATH]
```
